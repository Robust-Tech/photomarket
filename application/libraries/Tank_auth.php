...
class Tank_auth
{
	...
	{
		return $this->ci->db->select('role')->where('id',$r)->get('roles')->row()->role;
	}
	/**
    ...
    function create_author($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = 'Username already exists! Please choose another username.';

		} elseif (!$this->ci->users->is_email_available($email)) {
			$this->error = 'Email is already used by another user! Please choose another email.';
	...
	function admin_create_user($username, $email, $password, $firstname, $lastname, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');

...
class Template
{
	...
	function initialize($config = array())
	{
		...
		$this->_controller	= $this->_ci->router->fetch_module();
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['pagename']	= $this->_pagename;
	...
	public function title()
{
    ...
    if (func_num_args() == 1)
    {
        $page_segments = func_get_args();
        $this->_pagename = $page_segments;
    }
    ...
    private function _guess_pagename()
	{
		$this->_ci->load->helper('inflector');

		// Obviously no title, lets get making one
		$pagename_parts = array();

		// If the method is something other than index, use that
		if ($this->_method == 'index')
		{
			$pagename_parts[] = $this->_method;
		}
        elseif ( ! empty($this->_module))
		{
			$pagename_parts[] = $this->_module;
            
		} else
		{
			$pagename_parts[] = $this->_controller;
		}

		// Glue the title pieces together using the title separator setting
		$pagename = humanize(implode('', $pagename_parts));
